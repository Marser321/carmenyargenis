      ...
      <div className="grid grid-cols-[0.92fr_1.08fr] items-center gap-10">
        <div>
          <Stagger>
            ...
            <Reveal>
              <p className="mt-4 text-[16px] leading-relaxed text-ivory/70">{c.body}</p>
            </Reveal>
          </Stagger>

          {/* Paso 2 — el corazón del método: qué NO comprar */}
          <Step at={2} className="mt-7">
            <p className="text-[12px] font-semibold uppercase tracking-[0.18em] text-gold/80">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2.5">
              {c.filters.map((f) => (
                <div
                  key={f}
                  className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.05] px-4 py-2.5 transition-all duration-300 hover:-translate-y-0.5 hover:border-white/25"
                >
                  <span className="text-smoke">✕</span>
                  <span className="text-[13.5px] text-ivory/80">{f}</span>
                </div>
              ))}
            </div>
          </Step>
          ...
          <Step at={3} subtle className="mt-6">
            <p className="font-display text-xl text-gold">{c.closing}</p>
          </Step>
        </div>

        {/* Paso 1 — las 9 fases se construyen 01→09 */}
        <div>
          <p className="mb-3 text-[12px] font-semibold uppercase tracking-[0.18em] text-ivory/45">
