    ...
    >
      <dl className="divide-y divide-current/10">
        {lines.map((l, i) => (
          <Reveal key={l.label} delay={i * 0.05}>
            <div className="flex items-baseline justify-between gap-4 py-2.5">
              <div>
                <dt className={cn('text-[14px] font-medium', dark ? 'text-ivory/85' : 'text-charcoal/85')}>
                  {l.label}
                </dt>
                {l.hint && (
        ...
        <div className="flex items-baseline justify-between gap-4 pt-3.5">
          <dt className={cn('text-[13px] font-semibold uppercase tracking-[0.1em]', dark ? 'text-olive' : 'text-petrol')}>
            {totalLabel}
          </dt>
          <dd className={cn('font-display text-2xl font-semibold tabular-nums', dark ? 'text-ivory' : 'text-charcoal')}>
            {total}
          </dd>
        </div>
      </dl>
